...
currentAccount: any;
    ...
    loadAll() {
        this.uploadInfoService.query({
            page: this.page - 1,
            size: this.itemsPerPage,
            sort: this.sort()}).subscribe(
                (res: HttpResponse<UploadInfo[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/upload-info'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: UploadInfo) {
        return item.id;
    }
    registerChangeInUploadInfos() {
        this.eventSubscriber = this.eventManager.subscribe('uploadInfoListModification', (response) => this.loadAll());
    }
